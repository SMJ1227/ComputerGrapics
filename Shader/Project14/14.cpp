...
GLvoid drawScene() {
	...
	if (timerX == 1 || timerX == -1)	model = rotate(model, rotationAngleX, vec3(1.0f, 0.0f, 0.0f));  // X축을 기준으로 회전
	if (timerY == 1 || timerY == -1) model = rotate(model, rotationAngle, vec3(0.0f, 1.0f, 0.0f));
